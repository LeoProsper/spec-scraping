...
export async function GET(request: NextRequest) {
  try {
    ...
    // Ordenação (FASE 4: ordenar por prioridade por padrão)
    ...
    // Filtro de sem proposta (FASE 3)
    if (propostaStatus === 'sem') {
      // Buscar company_ids que TEM propostas
      ...
      if (hasProposals && hasProposals.length > 0) {
        ...
        // Excluir empresas que já têm propostas
        query = query.not('company_id', 'in', `(${companyIdsWithProposals.join(',')})`);
      }
      // Se não há propostas no banco, todas as empresas não têm proposta (não filtra)
    }
