...
function parseUserIntent(content: string): any {
  ...
  const radius = radiusMatch && radiusMatch[1] ? parseInt(radiusMatch[1]) : undefined;
...
async function processSearchInBackground(searchId: string, intent: any, supabase: any, _userId: string) {
  try {
    ...
    // Inserir empresas (Google Business + Receita Federal)
    const companies = result.places.map((place: any) => ({
      // Campos Google Business
      search_id: searchId,
      ...
      // Campos Receita Federal (26 campos)
      razao_social: place.razao_social,
      nome_fantasia: place.nome_fantasia,
      situacao_cadastral: place.situacao_cadastral,
      data_situacao_cadastral: place.data_situacao_cadastral,
      motivo_situacao_cadastral: place.motivo_situacao_cadastral,
      porte_empresa: place.porte_empresa,
      natureza_juridica: place.natureza_juridica,
      capital_social: place.capital_social,
      data_abertura: place.data_abertura,
      qsa: place.qsa,
      cnae_principal: place.cnae_principal,
      cnaes_secundarios: place.cnaes_secundarios,
      opcao_simples: place.opcao_simples,
      data_opcao_simples: place.data_opcao_simples,
      data_exclusao_simples: place.data_exclusao_simples,
      opcao_mei: place.opcao_mei,
      receita_logradouro: place.receita_logradouro,
      receita_numero: place.receita_numero,
      receita_complemento: place.receita_complemento,
      receita_bairro: place.receita_bairro,
      receita_cep: place.receita_cep,
      receita_municipio: place.receita_municipio,
      receita_uf: place.receita_uf,
      receita_email: place.receita_email,
      receita_telefones: place.receita_telefones,
      receita_raw_data: place.receita_raw_data,
