...
function CompanyItem({ place }: { place: Place }) {
  ...
  const handleToggle = (open: boolean) => {
    if (open) {
      console.log('📂 Item expandido:', {
        ...
        reviews_count_type: typeof place.reviews_count,
        has_rating: !!place.rating,
        has_reviews: !!place.reviews_count && place.reviews_count > 0,
        should_show_summary: !!place.rating && !!place.reviews_count && place.reviews_count > 0,
        has_rating_distribution: !!place.rating_distribution,
  ...
  const scrollImages = (direction: 'left' | 'right') => {
    ...
    const scrollAmount = 88; // 80px (width) + 8px (gap)
        ...
        <CollapsibleTrigger asChild>
          <div className="flex items-center gap-4 px-6 py-3 cursor-pointer">
            ...
            <div className="flex items-center gap-3 shrink-0">
              {/* Avaliação */}
              {(place.rating !== undefined && place.rating !== null) || (place.reviews_count !== undefined && place.reviews_count !== null && place.reviews_count > 0) ? (
                <div className="flex items-center gap-1.5 px-2 py-1 bg-yellow-50 dark:bg-yellow-950/20 rounded-md border border-yellow-200 dark:border-yellow-900">
                  <Star className="h-3.5 w-3.5 fill-yellow-400 text-yellow-400 shrink-0" />
                  <span className="font-semibold text-sm text-foreground whitespace-nowrap">
                    {place.rating !== undefined && place.rating !== null ? place.rating.toFixed(1) : '--'}
                  </span>
                ...
                <div className="flex items-center gap-1.5 px-2 py-1 bg-muted/50 rounded-md border border-border">
          ...
          <div className="px-6 pb-6 pt-2 bg-muted/10">
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                ...
                {/* EMPRESA LGBTQ+ */}
                {place.about && typeof place.about === 'string' && place.about.toLowerCase().includes('lgbtq') && (
                  <div className="flex items-center gap-3 text-sm">
                    <svg className="h-5 w-5 text-muted-foreground flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {/* RESUMO DAS AVALIAÇÕES DO GOOGLE - MOVIDO PARA ANTES DAS REVIEWS */}
                {place.rating && place.reviews_count && place.reviews_count > 0 && (
                    ...
                    <div className="flex items-start gap-8">
                      ...
                      <div className="flex-1 space-y-2.5">
                        {[5, 4, 3, 2, 1].map((stars) => {
                          ...
                          if (place.rating_distribution) {
                            const total = Object.values(place.rating_distribution).reduce((a, b) => a + b, 0);
                            const count = place.rating_distribution[stars as keyof typeof place.rating_distribution] || 0;
                            percentage = total > 0 ? (count / total) * 100 : 0;
                          } else {
                            // Fallback: estimativa baseada no rating médio
    ...
    <div className="w-full">
      ...
      <div className="border border-border rounded-lg overflow-hidden bg-background mt-4">
        {results.map((place, index) => (
          <CompanyItem key={place.place_id || index} place={place} />
        ))}
      </div>
    </div>
  );
