...
/**
 * Get AI configuration from database (or fallback to .env)
 */
async function getAIConfigFromDB(): Promise<AIConfigFromDB | null> {
  try {
    ...
    if (error || !data) {
      console.warn('[AI] No active config in database, using .env fallback');
      return null;
    }

    return data as AIConfigFromDB;
  } catch (error) {
    console.error('[AI] Failed to load config from database:', error);
    return null;
  }
}
 ...
 */
async function checkFeatureEnabled(mode: AIMode): Promise<boolean> {
  try {
    const supabase = getSupabaseServerClient();
    ...
    if (error || !data) {
      console.warn(`[AI] Feature flag not found for ${mode}, allowing by default`);
      return true;
    }

    return data.is_enabled;
  } catch (error) {
    console.error('[AI] Failed to check feature flag:', error);
...
async function logUsage(params: {
  ...
  try {
    ...
    // Don't throw - logging failure shouldn't break AI functionality
  }
}
...
export async function runAI(params: RunAIParams): Promise<RunAIResult> {
  ...
  const isEnabled = await checkFeatureEnabled(params.mode);
  if (!isEnabled) {
    const error = new AIError(
      `AI feature '${params.mode}' is disabled. Contact your administrator.`,
  ...
  // Get configuration (database first, then .env fallback)
  let apiKey: string;
  let apiBaseUrl: string;
  let modelDefault: string;
  let maxTokensDefault: number;
  ...
  if (dbConfig) {
    ...
    modelDefault = dbConfig.model_default;
    maxTokensDefault = dbConfig.max_tokens;
    timeoutMs = dbConfig.timeout_ms;
    temperature = dbConfig.temperature_default;
    
    console.log('[AI] Using config from database');
  } else {
    // Fallback to .env
    try {
      ...
      modelDefault = envConfig.openaiModelDefault;
      maxTokensDefault = envConfig.openaiMaxTokens;
      timeoutMs = envConfig.openaiTimeoutMs;
      temperature = 0.7;
      
      console.log('[AI] Using config from .env (fallback)');
    } catch (error) {
      const aiError = new AIError(
        'AI is not configured. Please configure in Admin Panel or set OPENAI_API_KEY in .env',
        AIErrorCode.CONFIG_ERROR,
        error
      );
