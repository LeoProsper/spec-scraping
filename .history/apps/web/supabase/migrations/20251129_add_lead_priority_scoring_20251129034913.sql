...
BEGIN
  ...
  SELECT 
    ...
    lead_status,
    category,
    (SELECT MAX(created_at) FROM company_interactions WHERE company_id = p_company_id)
  INTO 
    v_website,
    v_rating,
    v_reviews,
...
BEGIN
  ...
  FROM calculate_lead_priority(NEW.id);
...
BEGIN
  FOR company_record IN 
    SELECT id FROM companies WHERE lead_status NOT IN ('ganho', 'perdido')
  LOOP
    SELECT * INTO priority_result 
    FROM calculate_lead_priority(company_record.id);
