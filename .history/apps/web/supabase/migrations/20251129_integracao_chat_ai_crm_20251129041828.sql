...
-- Índice único composto: place_id + account_id
-- Garante que mesma empresa (place_id) não seja duplicada dentro de uma account
CREATE UNIQUE INDEX IF NOT EXISTS idx_companies_place_id_account 
ON public.companies(place_id, account_id)
WHERE place_id IS NOT NULL;

COMMENT ON INDEX idx_companies_place_id_account IS 
'Índice único que impede duplicação de empresas com mesmo place_id 
dentro da mesma account. Permite que diferentes accounts tenham a 
mesma empresa (isolamento via RLS).';
