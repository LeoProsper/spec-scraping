...
BEGIN
  ...
  SELECT 
    ...
    total_chamadas,
    total_reunioes
  INTO total_interacoes, total_chamadas, total_reunioes
  FROM public.interactions_por_empresa
  WHERE company_id = test_company_id;
  
  IF total_interacoes = 3 
     AND total_chamadas = 1 
     AND total_reunioes = 1 THEN
    RAISE NOTICE '✅ TESTE 8 PASSOU: View por_empresa retornou % interações (1 chamada, 1 reunião)', total_interacoes;
  ELSE
    RAISE EXCEPTION '❌ TESTE 8 FALHOU: Total = %, Chamadas = %, Reuniões = %', 
      total_interacoes, total_chamadas, total_reunioes;
  END IF;
END $$;
...
BEGIN
  ...
  SELECT 
    ...
    empresas_contatadas,
    deals_fechados
  INTO total_interacoes, empresas_contatadas, deals_fechados
  FROM public.interactions_por_usuario
  WHERE user_id = test_user_id;
  
  IF total_interacoes >= 3 
     AND empresas_contatadas >= 1 
     AND deals_fechados >= 1 THEN
    RAISE NOTICE '✅ TESTE 9 PASSOU: View por_usuario retornou % interações, % empresas, % deals', 
      total_interacoes, empresas_contatadas, deals_fechados;
  ELSE
