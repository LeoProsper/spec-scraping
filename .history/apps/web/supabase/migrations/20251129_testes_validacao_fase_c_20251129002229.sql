...
BEGIN
  ...
  WHERE place_id = 'place_teste_fase_c_lists' 
  LIMIT 1;

  IF test_company_id IS NULL THEN
    -- Buscar um search_id válido do usuário
    DECLARE
      test_search_id UUID;
    BEGIN
      SELECT id INTO test_search_id 
      FROM public.searches 
      WHERE user_id = test_user_id 
      LIMIT 1;

      -- Se não tiver search, criar um
      IF test_search_id IS NULL THEN
        INSERT INTO public.searches (user_id, query, status)
        VALUES (test_user_id, 'Teste FASE C', 'completed')
        RETURNING id INTO test_search_id;
      END IF;

      -- Criar empresa vinculada ao search
