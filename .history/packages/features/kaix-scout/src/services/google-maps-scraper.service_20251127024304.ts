...
export async function searchPlaces(
  ...
  try {
    // Use gosom/google-maps-scraper with web UI mode (simpler approach)
    // The scraper runs jobs automatically via the web interface
    // We'll create a job with minimal required fields and poll for results
    ...
    const jobPayload = {
      ...
      max_time: '5m',
      zoom: 15,
      ...(params.radius && { radius: params.radius * 1000 }), // Convert km to meters
    };

    console.log('[GoogleMapsScraperService] Job payload:', JSON.stringify(jobPayload));
    ...
    if (!createJobResponse.ok) {
      ...
      console.error('[GoogleMapsScraperService] Job creation failed:', errorText);
    ...
    const jobId = jobData.id || jobData.job_id || jobData.ID;
    ...
    // Step 2: Poll for results (wait up to 4 minutes - scraper needs time)
    const maxAttempts = 48; // 48 attempts * 5 seconds = 4 minutes
    let attempts = 0;
    ...
    while (attempts < maxAttempts) {
      ...
      const status = statusData.status || statusData.state || statusData.Status;

      console.log(`[GoogleMapsScraperService] Job status (attempt ${attempts}/${maxAttempts}):`, status);

      // Check if job is complete
      if (status === 'completed' || status === 'done' || status === 'finished' || status === 'Completed') {
    ...
    throw new GoogleMapsScraperError('Scraping job timeout after 4 minutes');
