...
export async function searchPlaces(
  ...
  try {
    ...
    const places: GoogleMapsPlace[] = data.businesses.map((business: any, index: number) => {
      const parsedRating = business.rating ? parseFloat(business.rating.toString()) : undefined;
      const parsedReviewsCount = business.reviews_count || (business.reviewCount ? parseInt(business.reviewCount.toString(), 10) : undefined);
      
      // Log para debug
      console.log(`[GoogleMapsScraperService] Business ${index + 1}:`, {
        name: business.name,
        rating: parsedRating,
        reviews_count: parsedReviewsCount,
        cnpj: business.cnpj,
        has_rating: !!parsedRating,
        has_reviews: !!parsedReviewsCount
      });
