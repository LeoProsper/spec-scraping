...
## 🗺️ ROADMAP TÉCNICO OBRIGATÓRIO

### FASE 1 — CORREÇÕES CRÍTICAS (SOBREVIVÊNCIA)

**Objetivo:** Garantir que o sistema não quebre em produção com 100-500 usuários

**Prazo:** 4-6 semanas

**Investimento Estimado:** 120-160 horas de desenvolvimento

#### 🔴 **Prioridade MÁXIMA:**

1. **Job Queue para Chat AI → CRM** (40h)
   - Implementar BullMQ ou Redis Queue
   - Mover integração `create_or_update_company_from_chat` para worker
   - API retorna response imediata + notifica quando processamento completar
   - **Risco se NÃO fizer:** Timeout em buscas grandes, UX péssima

2. **Rate Limiting** (16h)
   - Implementar rate limit por usuário: 10 buscas/minuto
   - Middleware de rate limiting em `/api/scout/search`
   - UI feedback quando limite atingido
   - **Risco se NÃO fizer:** Abuse derruba scraper

3. **Fallback Scraper** (32h)
   - Implementar retry com backoff exponencial
   - Fallback para Google Places API oficial (pago mas confiável)
   - Circuit breaker pattern para scraper
   - **Risco se NÃO fizer:** Downtime total se scraper cair

4. **Corrigir FK `onboarding`** (8h)
   - Migração: alterar `proposal_id` → `account_id`
   - Atualizar queries e RLS policies
   - **Risco se NÃO fizer:** Onboarding tracking quebrado

5. **Particionamento `messages`** (24h)
   - Implementar particionamento por `created_at` (monthly)
   - Migração de dados existentes
   - **Risco se NÃO fizer:** Performance degrada após 100k mensagens

**Total Fase 1:** **120 horas** (~3-4 semanas com 1 dev full-time)

---

### FASE 2 — ESTRUTURA DE MONETIZAÇÃO

**Objetivo:** Habilitar geração de receita e melhorar UX dos módulos principais

**Prazo:** 6-8 semanas

**Investimento Estimado:** 200-240 horas de desenvolvimento

#### 💰 **Módulos de Monetização:**

1. **Módulo Propostas Completo** (80h)
   - UI de criação de proposta (wizard multi-step)
   - Templates de proposta (texto + variáveis dinâmicas)
   - Geração de PDF automático (ReactPDF ou jsPDF)
   - Link público de proposta (proposal_url)
   - Status tracking (draft, sent, viewed, accepted, rejected)
   - Notificações quando proposta visualizada/aceita
   - **Retorno:** Módulo vendável, diferencial competitivo

2. **Integração Stripe + Módulo Pagamentos** (64h)
   - Integração Stripe Checkout
   - Webhook de confirmação de pagamento
   - UI de histórico de pagamentos
   - Dashboard financeiro (receita, MRR, churn)
   - **Retorno:** Monetização direta, controle financeiro

3. **Dashboard com Dados Reais** (40h)
   - Remover mocks de `DashboardDemo`
   - Queries de agregação reais (`user_stats` view)
   - Gráficos conectados ao banco (Recharts)
   - Filtros por período (7d, 30d, 90d, 12m)
   - KPIs principais: leads criados, taxa conversão, vendas fechadas
   - **Retorno:** Visibilidade executiva, decisões data-driven

4. **Loading States + UX Improvements** (32h)
   - Loading skeleton no Chat AI (estimativa 30-60s)
   - Progress bar durante scraping
   - Toast notifications consistentes
   - Error handling global com retry button
   - **Retorno:** UX profissional, reduz churn

5. **CI/CD Pipeline** (24h)
   - GitHub Actions: lint, type-check, build
   - Deploy automático Vercel/Railway
   - Preview deployments em PRs
   - Rollback automático em caso de erro
   - **Retorno:** Deploy seguro, zero downtime

**Total Fase 2:** **240 horas** (~6-8 semanas com 1 dev full-time)

---

### FASE 3 — ESCALA E AUTOMAÇÃO

**Objetivo:** Preparar sistema para 1k-10k usuários e automações inteligentes

**Prazo:** 8-12 semanas

**Investimento Estimado:** 280-320 horas de desenvolvimento

#### 🚀 **Otimizações de Escala:**

1. **Múltiplas Instâncias de Scraper** (48h)
   - Dockerizar scraper
   - Load balancer (Nginx ou Traefik)
   - 3-5 instâncias em paralelo
   - Health checks automáticos
   - **Retorno:** Capacidade 10x (500 buscas simultâneas)

2. **Arquivamento JSONB `searches.results`** (32h)
   - Estratégia: mover `results` para S3 após 6 meses
   - Trigger de arquivamento automático
   - API de recuperação de histórico arquivado
   - **Retorno:** Performance estável, custo reduzido

3. **Particionamento `product_events`** (24h)
   - Particionamento por `created_at` (monthly)
   - Retenção automática (12 meses)
   - Analytics em tempo real (stream processing)
   - **Retorno:** Queries de analytics 10x mais rápidas

4. **Módulo Análise de Website** (80h)
   - Scraper de websites (Puppeteer)
   - Scoring: SSL, responsividade, tempo de load, SEO básico
   - UI de visualização de análise
   - Trigger automático ao criar empresa
   - **Retorno:** Diferencial competitivo, lead scoring melhorado

5. **Sistema de Notificações** (64h)
   - Email notifications (Resend ou SendGrid)
   - In-app notifications (toast + inbox)
   - Notificações de follow-up pendente
   - Notificações de proposta visualizada/aceita
   - **Retorno:** Engajamento, reduz esquecimento de follow-ups

6. **Testes Automatizados** (48h)
   - Testes E2E críticos (Playwright): login, busca, CRM, proposta
   - Testes unitários backend (Vitest)
   - Coverage mínimo 60%
   - CI/CD rodando testes automaticamente
   - **Retorno:** Confiabilidade, reduz bugs em produção

7. **API Pública + Webhooks** (64h)
   - REST API pública documentada (Swagger)
   - Webhooks de eventos (lead_criado, proposta_aceita, venda_fechada)
   - Rate limiting por API key
   - Dashboard de integrações
   - **Retorno:** Integrações com Zapier, Make, CRMs externos

**Total Fase 3:** **360 horas** (~9-12 semanas com 1 dev full-time)

---

### RESUMO DO ROADMAP

| Fase | Objetivo | Prazo | Horas | Custo Estimado* |
|------|----------|-------|-------|-----------------|
| **Fase 1** | Sobrevivência | 4-6 sem | 120h | R$ 24.000 |
| **Fase 2** | Monetização | 6-8 sem | 240h | R$ 48.000 |
| **Fase 3** | Escala | 8-12 sem | 360h | R$ 72.000 |
| **TOTAL** | MVP → Scale-ready | 18-26 sem | 720h | **R$ 144.000** |
...
- ✅ Sistema não quebra com 100-500 usuários
- ✅ UX aceitável (não trava)
- ✅ Onboarding tracking funcional
- ✅ Proteção contra abuse

#### **Fase 2 (Monetização):**
- 💰 **Propostas vendáveis** → diferencial competitivo
- 💰 **Pagamentos integrados** → receita recorrente
- 📊 **Dashboard real** → decisões baseadas em dados
- 🎨 **UX profissional** → reduz churn
- 🚀 **Deploy automático** → zero downtime

#### **Fase 3 (Escala):**
- 📈 **10x capacidade** → 1k-10k usuários
- 🤖 **Automações inteligentes** → menos trabalho manual
- 🔌 **API pública** → integrações com ferramentas externas
- 🧪 **Testes automatizados** → confiabilidade
- 🌐 **Análise de website** → diferencial técnico

---

### CÁLCULO DE CAPACIDADE

| Métrica | Hoje | Após Fase 1 | Após Fase 2 | Após Fase 3 |
|---------|------|-------------|-------------|-------------|
| **Usuários Simultâneos** | 10 | 100 | 500 | 5000 |
| **Buscas/Minuto** | 5 | 50 | 100 | 500 |
| **Registros `messages`** | 1k | 100k | 500k | 10M |
| **Tempo de Response Chat** | 60s | 3s + async | 2s + async | 1s + async |
| **Uptime** | 90% | 95% | 98% | 99.5% |
| **Deploy Downtime** | 30min | 5min | 0min | 0min |

---

### PRIORIZAÇÃO RECOMENDADA

**Se você tem orçamento limitado, faça EXATAMENTE nesta ordem:**

1. **Job Queue** (40h) → UX não trava, crítico
2. **Rate Limiting** (16h) → Proteção contra abuse
3. **Fallback Scraper** (32h) → Sistema não quebra
4. **Dashboard Real** (40h) → Vende melhor
5. **Propostas UI** (80h) → Diferencial competitivo
6. **Stripe Integration** (64h) → Monetiza de verdade
7. **CI/CD** (24h) → Deploy sem medo
8. **Múltiplas Instâncias Scraper** (48h) → Escala
9. **Testes E2E** (48h) → Confiabilidade
10. **API Pública** (64h) → Integrações

**Total mínimo viável:** **296 horas** (itens 1-7) → **R$ 59.200**

---

## 🎯 RESPOSTAS FINAIS (EXECUTIVO)

### ➡️ "Esse sistema aguenta quantos usuários?"

**Hoje:** 10-50 usuários simultâneos (scraper é gargalo)  
**Após Fase 1:** 100-500 usuários simultâneos  
**Após Fase 2:** 500-1000 usuários simultâneos  
**Após Fase 3:** 1000-10000 usuários simultâneos  

### ➡️ "Dá para vender hoje?"

✅ **SIM**, mas:
- Como **Beta** para early adopters (preço 50% off)
- Deixando claro: Dashboard é mock, Propostas é manual
- Limite de 50 usuários pagantes até implementar Fase 1
- SLA de 90% uptime (sem garantia crítica)

### ➡️ "Onde ele quebra?"

🔴 **Pontos de Quebra:**
1. **Scraper cai** → Chat AI para completamente
2. **10+ buscas simultâneas** → Scraper trava
3. **100k mensagens** → Queries ficam lentas
4. **Export CSV grande** → Timeout após 30s
5. **Google bloqueia scraping** → Sistema offline

### ➡️ "Quanto custa arrumar?"

💰 **Investimento por Fase:**
- **Fase 1 (Sobrevivência):** R$ 24.000 (4-6 semanas)
- **Fase 2 (Monetização):** R$ 48.000 (6-8 semanas)
- **Fase 3 (Escala):** R$ 72.000 (8-12 semanas)
- **TOTAL:** R$ 144.000 (18-26 semanas)

**Mínimo viável:** R$ 59.200 (itens críticos apenas)

### ➡️ "Vale escalar ou reescrever?"

✅ **VALE ESCALAR**, porque:
- Arquitetura base é sólida (RLS, normalização, índices)
- Backend e Frontend bem estruturados
- Não há débito técnico irreversível
- Problemas são pontuais e resolvíveis

❌ **NÃO REESCREVER**, porque:
- 90% do código é aproveitável
- Reescrita custaria R$ 300k+ e 6+ meses
- Risco de perder features funcionais
...
Arquivos gerados:
- ✅ `DIAGNOSTICO_SPEC64_PARTE1.md` (71KB - Banco de dados + Backend + Visão Executiva)
- ✅ `DIAGNOSTICO_SPEC64_PARTE2.md` (65KB - Frontend + Chat AI + Segurança + Performance + Fluxo + Dívidas + Roadmap + Respostas Executivas)
