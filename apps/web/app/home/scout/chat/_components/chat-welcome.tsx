...
export function ChatWelcome() {
  ...
  console.log('🔄 ChatWelcome renderizado - searchIdParam:', searchIdParam, 'queryParam:', queryParam);
  ...
  const handleSearch = async (searchQuery?: string) => {
    ...
    try {
      ...
      // Salvar busca no histórico com título automático e resultados
      if (places.length > 0) {
        ...
        try {
          await fetch('/api/scout/searches', {
            ...
            body: JSON.stringify({
              ...
              status: 'completed',
              results: places, // Salvar dados completos
            }),
          });
        } catch (err) {
  ...
  // Load saved search results from history
  const loadedSearchIdRef = useRef<string | null>(null);
  useEffect(() => {
    // Se não tem searchId, reseta tudo e retorna
    if (!searchIdParam) {
      if (loadedSearchIdRef.current) {
        console.log('🧹 Limpando searchId anterior');
    ...
    // Se já carregou este searchId, não faz nada
    if (searchIdParam === loadedSearchIdRef.current) {
      console.log('✓ SearchId já carregado:', searchIdParam);
      return;
    }

    // Novo searchId diferente do anterior - limpa estados e carrega
    console.log('🔄 Mudança de searchId detectada:', loadedSearchIdRef.current, '→', searchIdParam);
    
    loadedSearchIdRef.current = searchIdParam;
    setSearchResults([]);
    setSearchError(null);
    setIsSearching(true);

    console.log('🔍 Carregando busca salva:', searchIdParam);

    fetch(`/api/scout/searches/${searchIdParam}`)
      .then(async (response) => {
        console.log('📡 Response status:', response.status);
        if (!response.ok) {
          const errorText = await response.text();
          console.error('❌ Response não OK:', errorText);
          throw new Error(`Busca não encontrada (${response.status})`);
        }
        const data = await response.json();
        const search = data.search;
        
        console.log('✅ Busca carregada:', {
          id: search?.id,
          query: search?.query,
        ...
        if (search && search.results && Array.isArray(search.results) && search.results.length > 0) {
          console.log('📊 Aplicando resultados ao state:', search.results.length, 'items');
          setSearchResults(search.results);
          setQuery(search.query || '');
        } else {
          console.warn('⚠️ Resultados vazios ou inválidos:', search);
          throw new Error('Resultados não disponíveis para esta busca');
        }
      })
      .catch((error) => {
        console.error('❌ Erro ao carregar busca salva:', error);
        loadedSearchIdRef.current = null; // Reset ref on error
        setSearchError(error instanceof Error ? error.message : 'Erro ao carregar busca');
      })
      .finally(() => {
        setIsSearching(false);
      });
  ...
  const hasExecutedSearch = useRef(false);
  useEffect(() => {
    if (queryParam && !hasExecutedSearch.current && !isSearching && searchResults.length === 0) {
      hasExecutedSearch.current = true;
      setQuery(queryParam);
